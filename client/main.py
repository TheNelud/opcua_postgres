...
if __name__ == "__main__":
    ...
    # schedule.every().hour.at("\"" + config['rate_1_hour']['cl_rate'] + "\"").do(create_client_post_to_post, client_hour)
    schedule.every().hour.at(":00").do(create_client_post_to_post, client_hour)
    # schedule.every().day.at("\"" + config['rate_1_day']['cl_rate'] + "\"").do(create_client_post_to_post, client_day)
    schedule.every().day.at("10:00:00").do(create_client_post_to_post, client_day)
